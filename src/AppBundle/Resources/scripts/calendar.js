...
{
    return '<tr class="calendarRow" onmouseover="showWeekData(' + weekIndex + ')">' + tableRow + '</tr>';
}
...
function enableDisableButtons()
{
    if(currentDisplayedMonth == 0)//disable 'go backwards' button if user is in first month
        $('#calendarLeftBtn').toggleClass('disabled');
    else if($('#calendarLeftBtn').hasClass('disabled'))//enable it if user went forward
        $('#calendarLeftBtn').toggleClass('disabled');

    if(currentDisplayedMonth == months.length - 1)//disable 'go forward' button if user is in last month
        $('#calendarRightBtn').toggleClass('disabled');
    else if($('#calendarRightBtn').hasClass('disabled'))//enable it if user went backwards
        $('#calendarRightBtn').toggleClass('disabled');
}
